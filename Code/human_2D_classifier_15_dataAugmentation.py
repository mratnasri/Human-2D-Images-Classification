...
dataAugmentation = '../../dataAugmentation_ver1/*.jpg'
categories_n = 200
...
def add_dataset(path, images, targets):
    img_names = glob(path)
    for fn in img_names:
        img = cv2.imread(fn)
        #img = load_img(fn)
        # print(img.shape)
        images.append(img)
        target = fn.split("\\")[-1].split("_")[0]
        targets.append(target)
    return images, targets


images, targets = load_dataset(all_images)
images, targets = add_dataset(dataAugmentation, images, targets)
...
plt.xlabel('epoch')
# plt.show()
plt.savefig('../../Outputs/model15_da_graph.png')

model.save('../Models/human_2D_model15_da.h5')
print("saved")
