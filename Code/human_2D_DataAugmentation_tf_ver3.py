...
sess=tf.Session()
all_images = '../../dataset_remaining/*.jpg'
...
def rotate_img(img):
    ...
    rot = skimage.transform.rotate(img, angle = random.randint(1,5), mode='edge')
    return rot
...
for j in range(len(images)):
    ...
    crop_write=tf.write_file('../../dataAugmentation_ver3/'+names[j].replace('.jpg', '_cropped.png'),cropped_img)
    ...
    writer = tf.write_file('../../dataAugmentation_ver3/'+names[j].replace('.jpg', '_rotated.png'),rotated)
    sess.run(crop_write)
    sess.run(writer)
