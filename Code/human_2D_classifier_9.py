...
x_train, x_test, y_train, y_test = train_test_split(
    images, targets, test_size=0.3, shuffle=True, stratify=targets)
...
def model_config():
    ...
    model.add(Conv2D(32, (3, 3), activation='relu',
                     kernel_initializer='he_uniform', input_shape=(480, 640, 3)))
...
def train_model(datax, datay, valx, valy):
    model = model_config()
    # fit model
    history = model.fit(datax, datay, epochs=35, batch_size=32,
...
plt.xlabel('epoch')
# plt.show()
plt.savefig('../../Outputs/model9_graph.png')

model.save('../Models/human_2D_model9.h5')
print("saved")
